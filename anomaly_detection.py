...
from utils import compute_AUC_EER
...
def main():
    ...
    for name, model in models:
        ...
        positive_scores = 1 / (1 + positive_scores)
        auc_list = []
        eer_list = []
        ...
        for filename in synthetic_filenames:
            ...
            negative_scores = 1 / (1 + negative_scores)
            auc, eer, fpr, tpr = compute_AUC_EER(positive_scores, negative_scores)
            eer_list.append(eer)
            auc_list.append(auc)
            if roc_data:
                # save ROC data
